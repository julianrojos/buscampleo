...
export default function JobCard({ job, isActive }: JobCardProps) {
  ...
  return (
    <article
      ...
      className={cn(
        'group flex cursor-pointer flex-col gap-3 border border-border bg-background p-4 text-left transition-colors hover:bg-muted/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring',
          ...
          <div className="min-w-0 flex-1 space-y-1">
            <div className="flex flex-wrap items-start justify-between gap-2">
              ...
              <div className="flex items-center gap-1" onClick={(event) => event.stopPropagation()}>
                ...
                >
                  {job.is_saved ? <BookmarkCheck className="size-3.5" /> : <Bookmark className="size-3.5" />}
                </Button>
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem
                      onSelect={() => hide(job.id)}
                    >
                      Ocultar
                    </DropdownMenuItem>
                    <DropdownMenuItem
                      onSelect={() => apply(job.id)}
                    >
